...
function removeFirstOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    ...
    result = str.slice(0, occur) + str.slice(occur + value.length);
  } else {
    result = str;
  }
  return result;
...
function sumOfCodes(str) {
  ...
  if (str) {
    result = str
      .split('')
      .reduce((accum, elem) => accum + elem.charCodeAt(), 0);
  } else {
    result = 0;
  }
  return result;
...
function isPalindrome(str) {
  const lowerString = str.toLowerCase().replace(/[ .,?!]/g, '');
  const reverString = lowerString.split('').reverse().join('');
  return lowerString === reverString;
}
 ...
 */
function findLongestWord(entence) {
  const arr = entence.split(' ').sort((a, b) => b.length - a.length);
  return arr[0];
}
